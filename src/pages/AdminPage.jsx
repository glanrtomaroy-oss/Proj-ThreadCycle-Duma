...
function AdminPage() {
  ...
  const [pendingDeleteShop, setPendingDeleteShop] = useState(null); // 🔹 for shop delete modal
  const [pendingDeleteComment, setPendingDeleteComment] = useState(null); // 🔹 for comment delete modal
  ...
  const handleDeleteShop = (shopId) => setPendingDeleteShop(shopId);
  const handleDeleteComment = (commentId) => setPendingDeleteComment(commentId);

  // ✅ Confirm actual deletion (shop)
  ...
  // ✅ Confirm actual deletion (comment)
  ...
  return (
    <div className="min-h-[calc(100vh-200px)] bg-gray-100 py-10">
      <div className="max-w-6xl mx-auto px-5">
        ...
        <div className="flex bg-white rounded-lg p-2 mb-8 shadow-lg gap-2">
          <button
            ...
            className={`flex-1 py-4 px-5 text-base font-medium rounded-md transition-all ${
              activeTab === 'shops'
                ? 'bg-[#2C6E49] text-white border border-[#2C6E49]'
        ...
        {activeTab === 'shops' && (
          <div className="bg-white rounded-lg p-8 shadow-lg">
            {/* (Form + Shop List same as before) */}
            {/* ... keep your existing form & list code here */}
          </div>
        )}
      ...
      {pendingDeleteComment && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-8 text-center">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Confirm Comment Deletion</h2>
            <p className="text-gray-600 mb-6">
            ...
            <div className="flex justify-center gap-4">
              <button
                ...
                className="px-6 py-3 bg-red-500 text-white rounded-md hover:bg-red-600 transition-all"
              >
                Yes, Delete
              </button>
              <button
                onClick={cancelDeleteComment}
                className="px-6 py-3 border-2 border-gray-400 text-gray-700 rounded-md hover:bg-gray-100 transition-all"
              >
                Cancel
              </button>
            </div>
